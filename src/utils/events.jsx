export  const events = [
  {
    id: 1,
    label: "Competition",
    title: "NUESA WEEK'23",
    img: "/events/3.jpeg",
    text: ["Big News Everyone🎤✨", "The wait is over!🥳", "An exciting week you can't afford to miss is happening soon💃💃", "Mark your calendar and get ready to have another unforgettable moment.", "Below are the activities for the week:", "📌Day 1- Awareness and Games( Monday 4th of Sept)", "📌Day 2- Dean's Cup ( Thursday 14th of Sept, 2023)", "📌Day 3- 7th Annual Conference (Wednesday 6th of Sept)", "📌Day 4- Community Service : Coleng Clean up( Thursday 7th of Sept)", "📌Day 5- Movie Night ( Friday 8th of Sept, 2023)", "Stay tuned for all the juicy details in the upcoming flyer for each day relating to the venue and time🤗"],
    location: "COLENG AUDITORIUM",
    date: "28th August 2023",
    time: "1PM"
  },
  {
    id: 2,
    label: "event",
    title: "The Future of Engineering Entrepreneurship in Nigeria: Challenges and Opportunities",
    img: "/events/5.jpeg",
    text: ["🎉 Exciting News! 🎉", "We're thrilled to announce the 7th Annual Conference themed 'The Future of Engineering Entrepreneurship in Nigeria: Challenges and Opportunities' 🚀", "🎙️ Have you seen our list of keynote speakers? This is a chance to converse with these speakers who bring a wealth of experience and expertise, and their insights are sure to inspire and enlighten all attendees.Don't miss this chance to gain valuable insights and also to connect with like-minded individuals.", "🌟 Join us for a day filled with inspiring talks,and networking opportunities with industry leaders and fellow students. Explore the dynamic world of engineering entrepreneurship and discover how it's shaping Nigeria's future.", "<a href='https://forms.gle/FKAEvbL1Chtbedon7' className='font-semibold underline text-primary1'> Register Here</a>", "#NUESACONFERENCE"],
    date: "6th Sept 2023",
    location: "COLENG AUDITORIUM"
  },
    {
      id: 3,
      label: "Event",
      title: "NUESA DINNER AND AWARDS NIGHT🎉",
      img: "/events/1.jpeg",
      text: [" You don't want to miss this!😌", "✨ Spectacular Performances", "🏆 Awards Presentation", "🎤 Renowned Speaker", "🎉 Amazing Surprises", "Mark your calendars and be there for an evening that promises to be the highlight of the session.", "PS : Remember, we'll be with our COLFHEC friends😍🥂", "For inquiries, contact", "<a href='https://wa.me/2348168544983' className='font-semibold underline'> Tewary </a>", "NUESA SOCIAL DIRECTOR"],
      
      date: "28th October, 2023",
      location: "PEAK OLAM",
      time: "8PM"
    },
    {
      id: 4,
      label: "event",
      title: "Launching a New Career In MEP",
      img: "/events/4.jpeg",
      text: ["📢 Join our FREE Webinar for Mechanical & Electrical Engineering Students & Fresh Graduates! 🌟#MepMarvels. ", "🚀 Discover Exciting Opportunities in MEP Careers", "🔧 Explore Mechanical Systems, Electrical, & Plumbing", "💼 Unlock Tips to Secure Internship Opportunities", "📚 Get Your FREE MEP Ebook", "🤝 Network with Industry Professionals", "Don't miss out on this chance to kickstart your MEP journey and become part of the #MepMarvels community! 🎓💡 Connect with peers, learn from experts, and shape your future in MEP. 🏗️🔌", "<a href='https://bit.ly/MepWebinar' className='font-semibold underline text-primary1'> Register Here</a>"],
      date: "6th Sept 2023",
      time: "4PM",
      location: "Zoom"
    },
    {
      id: 5,
      label: "event",
      title: "Securing Postgraduate Scholarships in Developed Countries!",
      img: "/events/3.jpeg",
      text: ["🎓 Join us for a game-changing 2-day webinar on 'Securing Postgraduate Scholarships in Developed Countries!'", "Be there to discover insider tips, application strategies, and success stories📚🌟.", "Don't miss this opportunity to unlock your dreams of studying abroad🤩!",  "<a href='https://forms.gle/YrtNT4Uryznnt26K8' className='font-semibold underline text-primary1'> Register Here</a>"],
      date: "12th Sept, 2023",
      location: "Zoom"
    },
  ];